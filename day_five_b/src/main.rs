...
impl CargoBay {
    ...
    fn move_cargo(&mut self, instruction: Instruction) {
        let start = (instruction.start - 1) as usize;
        let index = *&mut self.stacks[start].len() - (instruction.quantity as usize);
        let mut in_suspension = self.stacks[start].split_off(index);
        self.stacks[(instruction.end - 1) as usize].append(&mut in_suspension);
    }
