...
impl AssignmentPair {
    fn check_for_overlap(self) -> bool {
        let first_range = self.elf_a.0..=self.elf_a.1;
        let second_range = self.elf_b.0..=self.elf_b.1;

        first_range.contains(&self.elf_b.0)
            || first_range.contains(&self.elf_b.1)
            || second_range.contains(&self.elf_a.0)
            || second_range.contains(&self.elf_a.1)
    }
}
